...
-- PRODUCT TABLE
-- ================================================
-- ALTER TABLE to add new columns for stock_status and restock_date
-- Run these statements separately before using the updated procedures
--
...
BEGIN
    ...
    VALUES (p_sku, p_name, p_category, p_brand, p_price, p_stock, p_rating, 
        CASE WHEN p_stock = 0 THEN p_restock_date ELSE NULL END);
...
BEGIN
    SELECT product_id, sku, name, category, brand, price, stock, rating, restock_date,
        CASE 
            WHEN price < 1000 THEN 'cheap'
            WHEN price >= 1000 AND price <= 5000 THEN 'midrange'
            WHEN price > 5000 THEN 'expensive'
...
BEGIN
    ...
    SET name = p_name, price = p_price, stock = p_stock, rating = p_rating,
        restock_date = CASE 
            WHEN v_new_status = 'out_of_stock' THEN p_restock_date
            ELSE NULL
        END
...
BEGIN
    ...
    SET v_new_status = CASE 
        ...
        ELSE NULL END;
    UPDATE products SET stock = v_new_stock,
        restock_date = CASE 
            WHEN v_new_status = 'out_of_stock' THEN p_restock_date
            ELSE NULL
        END
...
BEGIN
    SELECT product_id, sku, name, brand, price, stock, rating, restock_date,
        CASE 
            WHEN price < 1000 THEN 'cheap'
            WHEN price >= 1000 AND price <= 5000 THEN 'midrange'
            WHEN price > 5000 THEN 'expensive'
