...
BEGIN
    ...
    VALUES (p_sku, p_name, p_category, p_brand, p_price, p_stock);
...
BEGIN
    SELECT product_id, sku, name, category, brand, price, stock
...
BEGIN
    UPDATE products
    SET name = p_name, price = p_price, stock = p_stock
...
BEGIN
    SELECT product_id, sku, name, brand, price, stock
    FROM products
...
-- USAGE EXAMPLES
-- ================================================
-- CALL sp_CreateProduct('SKU-001', 'Running Shoes', 'Footwear', 'Nike', 2999.00, 50);
-- CALL sp_GetProduct(1);
-- CALL sp_ListProducts();
-- CALL sp_UpdateProduct(1, 'Running Shoes Pro', 3499.00, 45);
