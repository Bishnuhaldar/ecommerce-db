...
BEGIN
    ...
    DECLARE v_new_stock_status VARCHAR(20);
    DECLARE v_old_stock_status VARCHAR(20);

    SELECT stock, stock_status INTO v_old_stock, v_old_stock_status FROM products WHERE product_id = p_product_id;

    UPDATE products
    SET name = p_name, price = p_price, stock = p_stock, rating = p_rating, restock_date = p_restock_date
    WHERE product_id = p_product_id;

    -- Recalculate stock_status
    IF p_stock > 10 THEN
        SET v_new_stock_status = 'in_stock';
    ELSEIF p_stock BETWEEN 1 AND 10 THEN
        SET v_new_stock_status = 'low_stock';
    ELSEIF p_stock = 0 THEN
        SET v_new_stock_status = 'out_of_stock';
    ELSE
        SET v_new_stock_status = NULL;
    END IF;

    UPDATE products SET stock_status = v_new_stock_status WHERE product_id = p_product_id;

    -- If stock_status changed from out_of_stock to something else, set restock_date to NULL
    IF v_old_stock_status = 'out_of_stock' AND v_new_stock_status <> 'out_of_stock' THEN
        UPDATE products SET restock_date = NULL WHERE product_id = p_product_id;
    END IF;
END $$
...
BEGIN
    ...
    IF v_old_stock + p_delta < 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient stock';
    END IF;
    SET v_new_stock = v_old_stock + p_delta;
    UPDATE products SET stock = v_new_stock, restock_date = p_restock_date WHERE product_id = p_product_id;
