...
CREATE PROCEDURE sp_CreateProduct(IN p_sku VARCHAR(100), IN p_name VARCHAR(255),
    IN p_category VARCHAR(100), IN p_brand VARCHAR(100), IN p_price DECIMAL(10,2), IN p_stock INT, IN p_rating DECIMAL(2,1), IN p_discount DECIMAL(5,2))
BEGIN
    DECLARE v_discount DECIMAL(5,2);
    SET v_discount = CASE WHEN p_discount < 0 THEN 0 WHEN p_discount > 100 THEN 100 ELSE p_discount END;
    INSERT INTO products (sku, name, category, brand, price, stock, rating, discount_percentage)
    VALUES (p_sku, p_name, p_category, p_brand, p_price, p_stock, p_rating, v_discount);
...
CREATE PROCEDURE sp_UpdateProduct(IN p_product_id INT, IN p_name VARCHAR(255),
    IN p_price DECIMAL(10,2), IN p_stock INT, IN p_rating DECIMAL(2,1), IN p_discount DECIMAL(5,2))
BEGIN
    DECLARE v_discount DECIMAL(5,2);
    SET v_discount = CASE WHEN p_discount < 0 THEN 0 WHEN p_discount > 100 THEN 100 ELSE p_discount END;
    UPDATE products
    SET name = p_name, price = p_price, stock = p_stock, rating = p_rating, discount_percentage = v_discount
